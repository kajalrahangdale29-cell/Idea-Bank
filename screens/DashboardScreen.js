...
const DashboardScreen = () => {
  ...
  // New state for scope & permissions
  const [scope, setScope] = useState('self'); // 'self' | 'team' | 'all'
  const [permissions, setPermissions] = useState({ canViewTeam: false, canViewAll: false });
  const [availableScopes, setAvailableScopes] = useState(['self']); // will update after dashboard fetch
  ...
  useFocusEffect(
    React.useCallback(() => {
      if (employeeSystemId && token) {
        // fetch using currently selected scope (if available)
  ...
  /**
   * fetchDashboard
   * - now accepts optional scope param (self/team/all)
   * - sets permissions and availableScopes from backend 'user' and 'scope' fields
   * - uses functional setState for cardData to avoid stale closures
   */
  const fetchDashboard = async (authToken, requestedScope = '') => {
    ...
    try {
      ...
      if (response.ok && jsonData.success) {
        ...
        // Update permissions & scope handling
        ...
        // Determine available scopes based on permissions (frontend-side)
        ...
        // If backend returned scope, use it, otherwise infer
        ...
        if (backendScope && scopesAvailable.includes(backendScope)) {
          ...
          // select default: admin->all, manager->team, else self
