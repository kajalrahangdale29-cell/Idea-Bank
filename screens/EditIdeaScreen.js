...
//   const existingFile = normalizeImagePath(ideaData.beforeImplementationImagePath || ideaData.imagePath || null);
...
//   const ideaStatus = ideaData.status || ideaData.ideaStatus || 'Draft';
//   const showDraftOption = ideaStatus.toLowerCase() === 'draft' && !isManagerEditing;
...
export default function EditIdeaScreen() {
  ...
  const pickImageFromGallery = async () => {
    ...
    if (!result.canceled && result.assets && result.assets.length) {
      ...
      setIsNewFile(true);
      const timestamp = Date.now();
      const extension = String(uri).split('.').pop()?.toLowerCase() || 'jpg';
      const cleanFileName = `image_${timestamp}.${extension}`;
      setFileName(cleanFileName);
      setShowFileOptions(false);
      validateFile(uri);
  ...
  const pickPDF = async () => {
    try {
      ...
      // DocumentPicker returns { type: 'success', uri, name } or { type: 'cancel' }
      if (result && result.type === 'success') {
        const uri = result.uri || result.fileUri || '';
        setFile(uri);
        setFileType('pdf');
        setIsNewFile(true);
        setFileName(result.name || 'document.pdf');
        setShowFileOptions(false);
        validateFile(uri);
      }
    } catch (error) {
