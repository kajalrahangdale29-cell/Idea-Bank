...
// import { View, Text, StyleSheet, TextInput, TouchableOpacity, Alert, ActivityIndicator, Modal, ScrollView, Linking, BackHandler } from "react-native";
...
//   if (s === "hold" || s.includes("hold")) return "#FFC107";
//   return "#9E9E9E";
// };
...
//       let filteredByStatus = items.filter(item => {
//         const status = (item.status || '').toLowerCase().trim();
//         return status.includes('hold') && !status.includes('approved') && !status.includes('reject');
//       });
...
//         filteredByDate = filteredByStatus.filter(item => {
//           if (!item.createdOn && !item.holdDate) return false;
//           const dateToCheck = item.holdDate || item.createdOn;
//           const createdTime = new Date(dateToCheck).getTime();
...
//           <Text style={styles.typeText}>{item.type || "On Hold"}</Text>
//         </View>
//       </View>
...
//         </View>
//         <View style={styles.row}>
//           <Text style={styles.label}>Hold Date:</Text>
//           <Text style={styles.value}>
//             {formatDate(item.holdDate || item.createdOn || item.creationDate)}
//           </Text>
//         </View>
//         <View style={styles.rowDetail}>
...
//           <Text style={styles.filterLabel}>Hold Date Range</Text>
...
//                 <Text style={styles.totalText}>Total Hold Ideas: {totalItems}</Text>
//               </View>
//             </>
//           )}
//         </ScrollView>
...
//             </TouchableOpacity>
//             <TouchableOpacity style={styles.rejectButton} onPress={() => openRemarkModal("reject")}>
...
//   searchSection: { backgroundColor: '#fff', flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: '#e0e0e0' },
...
const HoldScreen = () => {
  ...
  // FIX: Store approvalStage from list item at card-tap time
  const [selectedApprovalStage, setSelectedApprovalStage] = useState(null);
  ...
  const submitStatusUpdate = async () => {
    ...
    try {
      ...
      // CRITICAL FIX: The list item has "approvalStage": "BE Team" (with space),
      // but the server expects "BETeam" (no space, camelCase) as seen in remarks.
      // The detail API returns "currentApprovalStage": "BETeam" which is the EXACT
      // value UpdateStatus uses to find the approval record in the DB.
      // Sending "BE Team" causes 404 "Implementation approval record not found".
      const recordId = ideaDetail.id.toString();
      const approvalStage = ideaDetail.currentApprovalStage
        || selectedApprovalStage
        || 'Manager';
      ...
      console.log('Submitting hold action:', { recordId, approvalStage, status: statusMap[remarkType] });
  ...
  // FIX: Also clear selectedApprovalStage on modal close
  const closeModal = () => {
    setSelectedIdea(null);
    setIdeaDetail(null);
    setEmployeeInfoExpanded(false);
  ...
  const renderIdeaCard = ({ item }) => (
    <TouchableOpacity
      ...
      onPress={() => {
        ...
        // FIX: Capture approvalStage from the list item HERE before fetching detail.
        // The detail API does not always return approvalStage, and the old lookup
        // via ideas.find() was broken due to ID type mismatches between list & detail APIs.
        setSelectedApprovalStage(item.approvalStage || 'Manager');
        fetchIdeaDetail(ideaId);
      }}
    >
      <View style={styles.cardHeader}>
...
const styles = StyleSheet.create({
  ...
  closeButtonImage: { position: 'absolute', top: 50, right: 20, zIndex: 10, backgroundColor: 'rgba(255,255,255,0.2)', borderRadius: 22, width: 44, height: 44, justifyContent: "center", alignItems: "center" },
  fullImage: { width: "80%", height: "60%" },
  actionButtonsContainer: { flexDirection: 'row', backgroundColor: '#FFFFFF', paddingHorizontal: 10, paddingVertical: 14, paddingTop: 12, paddingBottom: 24, borderTopWidth: 1, borderTopColor: '#e0e0e0', elevation: 10, shadowColor: '#000', shadowOffset: { width: 0, height: -3 }, shadowOpacity: 0.15, shadowRadius: 5, gap: 8 },
  approveButton: { flex: 1, backgroundColor: '#4CAF50', paddingVertical: 12, borderRadius: 8, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 6, elevation: 2 },
  rejectButton: { flex: 1, backgroundColor: '#F44336', paddingVertical: 12, borderRadius: 8, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 6, elevation: 2 },
  actionButtonText: { color: '#fff', fontSize: 13, fontWeight: 'bold', textTransform: 'uppercase' },
  remarkModalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', alignItems: 'center', padding: 20 },
