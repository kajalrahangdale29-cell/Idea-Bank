...
const getStatusColor = (status) => {
  ...
  if (s === "approved" || s === "closed") return "#607D8B";
  if (s === "rejected") return "#F44336";
  if (s === "hold" || s === "on hold" || s.includes("hold by")) return "#FFC107";
  return "#9E9E9E";
};
...
const HoldScreen = () => {
  ...
  const fetchHoldIdeas = async (from, to) => {
    try {
      ...
      // CRITICAL FIX: Filter only "hold" status ideas - case insensitive and comprehensive
      let filteredByStatus = items.filter(item => {
        const status = (item.status || '').toLowerCase().trim();
        // Only show if status contains "hold" but NOT "approved" or "rejected"
  ...
  const submitStatusUpdate = async () => {
    ...
    try {
      ...
      console.log("ideaId:", ideaIdValue);
      console.log("status:", statusMap[remarkType]);
      console.log("approvalstage:", originalIdea?.approvalStage || ideaDetail?.approvalStage || "Manager");
      ...
      if (response.data && (response.data.success === true || response.status === 200)) {
        ...
        Alert.alert("Success", successMessage, [{ 
          text: "OK", 
          onPress: () => {
            // Force refresh the list after closing alert
            fetchHoldIdeas();
          } 
        }]);
      ...
      if (error.response?.status === 404) {
        errorMessage = "API endpoint not found. Please check the UPDATE_STATUS_URL in api.js";
      ...
      <View style={styles.cardContent}>
        ...
        <View style={styles.row}>
          ...
          <Text style={styles.value}>
            {formatDateToDDMMYYYY(item.holdDate || item.createdOn || item.creationDate)}
          </Text>
        </View>
        <View style={styles.row}>
    ...
    <View style={styles.container}>
      ...
      <Modal visible={!!selectedIdea} animationType="slide">
        <View style={styles.fullModal}>
                  ...
                  <View style={styles.rowDetail}>
                    ...
                    <Text 
                      style={[
                        ...
                        { backgroundColor: getStatusColor(ideaDetail.ideaStatus || ideaDetail.status) }
                      ]}
                    >
          ...
          {/* Action Buttons - Only Direct Approve and Reject */}
          <View style={styles.actionButtonsContainer}>
            <TouchableOpacity 
              style={styles.approveButton} 
              onPress={() => openRemarkModal("approve")}
            >
            ...
            <TouchableOpacity 
              ...
              onPress={() => openRemarkModal("reject")}
            >
