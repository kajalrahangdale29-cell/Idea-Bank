...
const getImageUrlVariations = (path) => {
  if (!path) return [];
  ...
  const alternate = getAlternateImageUrl(normalized);
  if (alternate && alternate !== normalized) urls.push(alternate);

  if (path.includes('/uploads/')) {
    const withoutUploads = path.replace('/uploads/', '/');
    const url1 = normalizeImagePath(withoutUploads);
    if (url1 && !urls.includes(url1)) urls.push(url1);
    
    const url2 = getAlternateImageUrl(url1);
...
export default function ManagerEditIdeaScreen() {
  ...
  useEffect(() => {
    ...
    if (rawExisting && typeof rawExisting === 'string') {
      const variations = getImageUrlVariations(rawExisting);
      ...
      if (variations.length > 0) {
        setFile(variations[0]);
        setDisplayFile(variations[0]);
        
  
        const isPDF = String(rawExisting).toLowerCase().includes('.pdf');
        setFileType(isPDF ? 'pdf' : 'image');
        
    
        try {
          if (isPDF) {
            setFileName('Existing PDF Document');
          } else {
            const urlParts = String(rawExisting).split('/');
            const filename = urlParts[urlParts.length - 1] || '';
            const decoded = decodeURIComponent(filename || '');
            setFileName(decoded.replace(/\s+/g, ' ').trim() || 'Existing Image');
          }
        } catch (e) {
          setFileName(isPDF ? 'Existing PDF Document' : 'Existing Image');
        }
      }
    }
  }, [ideaData.beforeImplementationImagePath, ideaData.imagePath]);
  ...
  const handleImageError = useCallback(() => {
    console.log('Image load error for URL:', displayFile);
    
    if (!isNewFile && imageUrlVariations.length > 0) {
      const nextIndex = currentUrlIndex + 1;
      
      if (nextIndex < imageUrlVariations.length) {
        console.log('Trying fallback URL:', imageUrlVariations[nextIndex]);
        setCurrentUrlIndex(nextIndex);
        setDisplayFile(imageUrlVariations[nextIndex]);
  ...
  }, [isNewFile, imageUrlVariations, currentUrlIndex, displayFile]);
  ...
  const pickImageFromGallery = async () => {
    try {
      ...
      if (!result.canceled && result.assets && result.assets[0]) {
        ...
        console.log('Gallery - Original URI:', asset.uri);

        setFile(asset.uri);
        setDisplayFile(asset.uri);
        setImageLoadError(false);
        setFileType('image');
        setIsNewFile(true);
        setImageUrlVariations([]);
  ...
  const pickImageFromCamera = async () => {
    try {
      ...
      if (!result.canceled && result.assets && result.assets[0]) {
        const asset = result.assets[0];
        console.log('Camera - Original URI:', asset.uri);
  ...
  const pickPDF = async () => {
    try {
      ...
      if (result.type === 'success' || !result.canceled) {
        ...
        setDisplayFile(selectedFile.uri);
        setFileType('pdf');
        setIsNewFile(true);
        setImageUrlVariations([]);
  ...
  const openPDF = () => {
    if (displayFile && fileType === 'pdf') {
      if (displayFile.startsWith('http://') || displayFile.startsWith('https://')) {
        Linking.openURL(displayFile).catch(() => {
                  ...
                  >
                    {!imageLoadError ? (
                      ...
                      <View style={styles.imageErrorContainer}>
                        ...
                        <Text style={styles.imageErrorSubtext}>File may not be available on server</Text>
                      </View>
                    )}
                  </ScrollView>
