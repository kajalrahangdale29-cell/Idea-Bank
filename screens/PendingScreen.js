...
import { PENDING_APPROVALS_URL, IDEA_DETAIL_URL } from "../src/context/api";
...
export default function PendingScreen() {
  ...
  const fetchIdeaDetail = async (ideaId) => {
        ...
        try {
          ...
          // Find idea by ideaId, id, or ideaNumber
          const idea = ideas.find(i => 
            i.ideaId === ideaId || 
            i.id === ideaId || 
            i.ideaNumber === ideaId ||
            (typeof ideaId === 'string' && i.ideaNumber && i.ideaNumber === ideaId)
          );
          
          if (idea) {
            // Enhanced timeline creation based on actual API response structure
            const timeline = [];
            ...
            if (idea.creationDate) {
              timeline.push({ 
                ...
                description: 'Idea was created and submitted to the system'
              });
            }
            
            if (idea.status === 'Pending' && idea.approvalStage) {
              timeline.push({
                status: `Pending at ${idea.approvalStage}`,
                date: idea.creationDate,
                description: `Idea is currently pending approval at ${idea.approvalStage} stage`
              });
            }
            
            // Add any additional timeline events if available in API
