...
import { View, Text, StyleSheet, ScrollView, TouchableOpacity, Alert, Modal, Image, ActivityIndicator, TextInput, Vibration, Platform, Linking } from 'react-native';
    ...
    <ScrollView contentContainerStyle={styles.container}>
      {ideas.length === 0 ? (
        ...
        ideas.map((idea, index) => {
          return (
            <TouchableOpacity key={index} activeOpacity={0.8} style={styles.cardContainer} onPress={() => fetchIdeaDetail(idea.id)}>
              <View style={styles.cardHeader}>
                <Text style={styles.ideaNumber} numberOfLines={2}>{idea.ideaNumber || "N/A"}</Text>
                <View style={styles.typeTag}><Text style={styles.typeText}>{idea.ideaType || "N/A"}</Text></View>
              </View>
              <View style={styles.cardContent}>
                <View style={styles.rowDetail}><Text style={styles.label}>Idea Description:</Text><Text style={styles.value} numberOfLines={2}>{idea.ideaDescription || "N/A"}</Text></View>
                <View style={styles.rowDetail}><Text style={styles.label}>Proposed Solution:</Text><Text style={styles.value} numberOfLines={2}>{idea.proposedSolution || "N/A"}</Text></View>
                <View style={styles.row}><Text style={styles.label}>Created Date:</Text><Text style={styles.value}>{formatDate(idea.ideaCreationDate)}</Text></View>
                <View style={styles.rowDetail}>
                  <Text style={styles.label}>Status:</Text>
                  <View style={{ flex: 1, alignItems: 'flex-end' }}>
                    <Text style={[styles.statusBadge, { backgroundColor: getStatusColor(idea.ideaStatus) }]} numberOfLines={2}>
                      {idea.ideaStatus || "N/A"}
                    </Text>
                  </View>
                </View>
      ...
      {loadingDetail && (
        <View style={styles.loadingOverlay}><ActivityIndicator size="large" color="#2c5aa0" /></View>
      )}
      ...
      <Modal visible={showImplementationForm && !!selectedIdea} animationType="slide">
        <View style={styles.fullModal}>
          ...
          <ImplementationForm ideaDetail={ideaDetail} onClose={closeModal} refreshIdeas={refreshIdeas} />
        </View>
      </Modal>
              ...
              <>
                ...
                {ideaInfoExpanded && (
                  <View style={styles.cardDetail}>
                    ...
                    <View style={styles.rowDetailWithBorder}><Text style={styles.labelDetail}>Status:</Text><Text style={[styles.statusBadgeDetail, { backgroundColor: getStatusColor(ideaDetail.ideaStatus) }]}>{ideaDetail.ideaStatus || "N/A"}</Text></View>
                      ...
                      <View style={styles.cardDetail}>
                        ...
                        {ideaDetail.implementationCycle?.beforeImplementationImagePath && (
                          <View style={styles.implementationImageSection}>
                            ...
                            <TouchableOpacity onPress={() => openImagePreview(ideaDetail.implementationCycle.beforeImplementationImagePath)}>
                              <Image source={{ uri: normalizeImagePath(ideaDetail.implementationCycle.beforeImplementationImagePath) }} style={styles.implementationImage} />
                            </TouchableOpacity>
                          </View>
                        )}
                        ...
                        {/* {ideaDetail.implementationCycle?.afterImplementationImagePath && (
                          <View style={styles.implementationImageSection}>
                            ...
                            <TouchableOpacity onPress={() => openImagePreview(ideaDetail.implementationCycle.afterImplementationImagePath)}>
                              <Image source={{ uri: normalizeImagePath(ideaDetail.implementationCycle.afterImplementationImagePath) }} style={styles.implementationImage} />
                            </TouchableOpacity>
                          </View>
                        )} */}
                ...
                <View style={styles.cardDetail}>
                  ...
                  {(() => {
                    ...
                    return remarks.map((remark, index) => (
                      <RemarksCard key={index} title={remark.approverName || remark.title || "Unknown"} comment={remark.comments || remark.comment || "No comment"} date={remark.approvalDate || remark.date ? formatDateTime(remark.approvalDate || remark.date) : ""} />
                    ));
                  })()}
                </View>
                  ...
                  return (canEdit || canDelete || inImplementation) && (
                    <View style={styles.buttonRow}>
                      {(canEdit || inImplementation) && (
                        <TouchableOpacity style={[styles.button, styles.editButton]} onPress={() => handleEdit(ideaDetail)}>
                          <Ionicons name="create-outline" size={18} color="#fff" />
                          <Text style={styles.buttonText}>{inImplementation ? 'Submit Implementation' : 'Edit'}</Text>
                        </TouchableOpacity>
                      )}
                      {canDelete && (
                        <TouchableOpacity style={[styles.button, styles.deleteButton]} onPress={() => { deleteIdea(ideaDetail, () => { closeModal(); refreshIdeas(); }); }}>
              ...
              <View style={styles.timelineContainer}>
                {ideaDetail?.timeline && Array.isArray(ideaDetail.timeline) && ideaDetail.timeline.length > 0 ? (
                  ideaDetail.timeline.map((item, idx) => (
                    <TimelineItem key={idx} status={item.status || item.approvalStage || "N/A"} date={item.date || item.approvalDate} description={item.description || item.comments} isLast={idx === ideaDetail.timeline.length - 1} />
                  ))
                ) : (
...
export default function MyIdeasScreen() {
  ...
  useEffect(() => { const unsubscribe = navigation.addListener('focus', () => { fetchIdeas(); }); return unsubscribe; }, [navigation]);
  useEffect(() => { if (route.params?.newIdea) { if (route.params?.showDraftMessage) { Alert.alert('Draft Saved', 'Your idea has been saved as a draft.'); } fetchIdeas(); } }, [route.params?.newIdea, route.params?.showDraftMessage]);
  useEffect(() => { if (route.params?.refreshIdeas) { fetchIdeas(); } }, [route.params?.refreshIdeas]);
  ...
  const editIdea = (idea) => { navigation.navigate("EditIdea", { ideaId: idea.id, ideaData: idea }); };

  const filterIdeas = (status) => {
    return ideas.filter((idea) => {
      const s = (idea.ideaStatus || "").toLowerCase().trim();
      switch (status.toLowerCase()) {
        case "pending": return ["pending", "approval pending", "rm approval pending", "under review by be team", "be_team approval pending", "published", "draft", "ready for implementation", "approved by be team", "approved by be team - ready for implementation"].includes(s);
        case "approved": return s === "closed";
        case "hold": return ["hold", "idea hold by be team", "implementation hold by be team", "idea hold by manager", "implementation hold by manager"].includes(s);
        case "rejected": return ["rejected", "implementation rejected by manager", "idea rejected by be team", "implementation rejected by be team", "idea rejected by manager"].includes(s);
  ...
  return (
    <Tab.Navigator screenOptions={{ tabBarActiveTintColor: "#000000", tabBarInactiveTintColor: "#333333", tabBarStyle: { backgroundColor: "#CED8E7" }, tabBarIndicatorStyle: { backgroundColor: "#5A6FAE" }, tabBarLabelStyle: { fontSize: 14, fontWeight: "bold" }, tabBarScrollEnabled: true }}>
...
function ImplementationForm({ ideaDetail, onClose, refreshIdeas }) {
  ...
  const pickImageFromCamera = async () => {
    ...
    if (!permissionResult.granted) { Alert.alert('Camera permission required!'); return; }
  ...
  const handleSubmit = async () => {
    if (!implementationDetails.trim()) { Alert.alert('Error', 'Please enter implementation details'); return; }
    if (!outcomesBenefits.trim()) { Alert.alert('Error', 'Please enter outcome/benefits achieved'); return; }
    if (!afterImage) { Alert.alert('Error', 'Please upload after implementation image'); return; }
    ...
    try {
      ...
      if (afterImage && afterImageName) {
        ...
        formData.append('AfterImplementationImage', { uri: fileInfo.uri, type: mimeType, name: cleanName });
      }

      const response = await axios.post(SUBMIT_URL, formData, { headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'multipart/form-data' } });

      if (response.data?.success) {
        const pattern = [0, 100, 50, 100];
        Vibration.vibrate(pattern);
        Alert.alert('Success', response.data?.message || 'Implementation submitted successfully!', [{ text: 'OK', onPress: () => { refreshIdeas(); onClose(); } }]);
      } else {
    ...
    <ScrollView contentContainerStyle={styles.modalScrollContent}>
      ...
      {ideaInfoExpanded && (
        <View style={styles.cardDetail}>
          ...
          <View style={styles.rowDetailWithBorder}>
            ...
            <Text style={[styles.statusBadgeDetail, { backgroundColor: getStatusColor(ideaDetail.ideaStatus) }]}>{ideaDetail.ideaStatus || "N/A"}</Text>
          </View>
          <View style={styles.rowDetailWithBorder}><Text style={styles.labelDetail}>Idea Description:</Text><Text style={styles.valueDetail}>{ideaDetail.ideaDescription || "N/A"}</Text></View>
          <View style={styles.rowDetailWithBorder}><Text style={styles.labelDetail}>Proposed Solution:</Text><Text style={styles.valueDetail}>{ideaDetail.proposedSolution || "N/A"}</Text></View>
          <View style={styles.rowDetailWithBorder}><Text style={styles.labelDetail}>Process Improvement:</Text><Text style={styles.valueDetail}>{ideaDetail.tentativeBenefit || "N/A"}</Text></View>
          <View style={styles.rowDetailWithBorder}><Text style={styles.labelDetail}>Team Members:</Text><Text style={styles.valueDetail}>{ideaDetail.teamMembers || "N/A"}</Text></View>
      ...
      {ideaDetail.beforeImplementationImagePath && (
        <View style={styles.cardDetail}>
          ...
          <View style={styles.implementationImageSection}>
            <Image source={{ uri: ideaDetail.beforeImplementationImagePath }} style={styles.implementationImage} resizeMode="cover" />
          </View>
        </View>
      )}
      ...
      <TouchableOpacity style={styles.collapsibleHeader} onPress={() => setImplementationFormExpanded(!implementationFormExpanded)} activeOpacity={0.7}>
        <Text style={styles.collapsibleHeaderText}>Submit Implementation Details</Text>
        <Ionicons name={implementationFormExpanded ? "chevron-up" : "chevron-down"} size={24} color="#2c5aa0" />
      </TouchableOpacity>
      ...
      <Modal visible={showFileOptions} transparent={true} animationType="slide">
        <View style={styles.imageOptionsContainer}>
          <View style={styles.imageOptionsContent}>
            ...
            <TouchableOpacity style={[styles.imageOptionButton, styles.cancelButton]} onPress={() => setShowFileOptions(false)}>
...
const styles = StyleSheet.create({
  ...
  buttonRow: { flexDirection: "row", justifyContent: "center", marginTop: 20, gap: 12 },
  button: { flexDirection: 'row', alignItems: 'center', paddingVertical: 12, paddingHorizontal: 24, borderRadius: 10, gap: 6, elevation: 2 },
  editButton: { backgroundColor: "#607D8B" },
  ...
  timelineButtonHeader: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#e3f2fd', paddingHorizontal: 16, paddingVertical: 10, borderRadius: 8, borderWidth: 1, borderColor: '#2c5aa0', marginBottom: 12 },
  ...
  textInputArea: { borderWidth: 1, borderColor: '#ddd', borderRadius: 6, padding: 12, fontSize: 14, color: '#333', backgroundColor: '#fafafa', textAlignVertical: 'top', minHeight: 100 },
  fileInputRow: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#F0F2F5', borderRadius: 8, borderWidth: 1, borderColor: '#ddd', overflow: 'hidden' },
  chooseFileButton: { backgroundColor: '#e0e0e0', paddingHorizontal: 16, paddingVertical: 12, borderRightWidth: 1, borderRightColor: '#ccc' },
  chooseFileText: { color: '#333', fontSize: 14, fontWeight: '500' },
  fileNameDisplay: { flex: 1, paddingHorizontal: 12, color: '#666', fontSize: 14 },
  eyeIconContainer: { marginTop: 8, flexDirection: 'row', alignItems: 'center', alignSelf: 'flex-start' },
  previewText: { marginLeft: 6, color: '#2196F3', fontSize: 14, fontWeight: '500' },
  pdfInfoContainer: { marginTop: 8, flexDirection: 'row', alignItems: 'center', backgroundColor: '#FFF3E0', paddingHorizontal: 12, paddingVertical: 8, borderRadius: 6, alignSelf: 'flex-start' },
  ...
  imageOptionsContent: { backgroundColor: '#fff', borderTopLeftRadius: 20, borderTopRightRadius: 20, padding: 20, paddingBottom: 40 },
  imageOptionsTitle: { fontSize: 18, fontWeight: 'bold', color: '#333', marginBottom: 20, textAlign: 'center' },
  imageOptionButton: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#F0F2F5', padding: 16, borderRadius: 12, marginBottom: 12 },
  ...
  noDataText: { textAlign: 'center', color: '#999', fontSize: 14, fontStyle: 'italic', paddingVertical: 12 },
  noDataContainer: { alignItems: 'center', paddingVertical: 30 },
  submitImplementButton: { backgroundColor: '#1976D2', paddingVertical: 14, borderRadius: 8, alignItems: 'center', marginBottom: 16, elevation: 3, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.2, shadowRadius: 3 },
  submitImplementButtonText: { color: '#fff', fontSize: 16, fontWeight: 'bold' },
  submitButtonDisabled: { backgroundColor: '#9E9E9E' },
  imagePreviewModal: { flex: 1, backgroundColor: 'rgba(0,0,0,0.95)' },
  imagePreviewHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 20, paddingTop: 50, paddingBottom: 15 },
  zoomControls: { flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(255,255,255,0.2)', borderRadius: 20, paddingHorizontal: 10, paddingVertical: 5 },
  zoomButton: { padding: 8 },
  zoomText: { color: '#fff', fontSize: 14, fontWeight: 'bold', marginHorizontal: 15, minWidth: 50, textAlign: 'center' },
  imageScrollContent: { flex: 1, justifyContent: 'center', alignItems: 'center' },
  fullImagePreview: { width: 350, height: 500 },
});