...
export default function LoginScreen({ navigation }) {
  ...
  const handleLogin = () => {
    ...
    if (!password.trim()) { // Password empty check
      Alert.alert('Error', 'Password cannot be empty');
      return;
    }
    setUser({ name: 'Kajal', id: employeeId });
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: '#0f526a' },
  topBackground: { height: height * 0.4, backgroundColor: '#0f526a', alignItems: 'center', justifyContent: 'center' },
  logoColumn: { alignItems: 'center', justifyContent: 'center', flex: 1, width: '100%', marginTop: 40, paddingLeft: 45 },
  ibLogo: { width: 400, height: 200, marginBottom: 90, resizeMode: 'contain' },
  bottomBackground: { height: height * 0.7, backgroundColor: '#fff' },
  loginCard: { position: 'absolute', top: height * 0.31, height: height * 0.6, alignSelf: 'center', width: '90%', backgroundColor: '#fff', borderRadius: 30, padding: 25, elevation: 10, shadowColor: '#000', shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.15, shadowRadius: 8 },
